...
用户登录后，通常需要获取会话对象以便以某种方式使用其中的数据。一个常见用例是显示用户的个人资料图片或其他用户信息。
...
如果您希望从OAuth提供商扩展会话字段，请参阅我们的["扩展会话"指南](/guides/extending-the-session)。

<Callout>
  默认情况下，向会话端点发送的GET请求会自动返回防止缓存的头部信息。
</Callout>
