...
Postmark 提供商通过电子邮件发送包含验证令牌 URL 的"魔法链接"，可用于登录。

在支持 OAuth 服务登录的同时增加邮件登录方式，为用户在无法访问 OAuth 账户时（例如账户被锁定或删除）提供了备用登录途径。

Postmark 提供商可单独使用，也可与一个或多个 OAuth 提供商配合使用。
...
<Callout type="warning">
  Postmark 提供商同时支持 JSON Web Token
  和数据库管理的会话，但**必须配置数据库**才能使用。若不使用数据库，则无法启用电子邮件登录功能。
</Callout>

## 配置

1. 首先，您需要[在 Postmark 账户中添加域名](https://account.postmarkapp.com/sign_up)。这是 Postmark 的强制要求，该域名将作为 `from` 提供商选项中使用的地址域名。

2. 接着，您需要在 [Postmark 仪表板](https://account.postmarkapp.com/api_tokens) 生成 API 密钥。可将此 API 密钥保存为 `AUTH_POSTMARK_KEY` 环境变量。

```sh
AUTH_POSTMARK_KEY=abc
```

若将环境变量命名为 `AUTH_POSTMARK_KEY`，提供商将自动识别，您的 Auth.js 配置对象会更简洁。如需使用其他名称，则需在 Auth.js 配置中手动传递给提供商。
...
通过向 `Postmark()` 传递自定义函数作为 `sendVerificationRequest` 选项，可完全自定义发送的登录邮件内容。
...
以下示例展示了我们内置的 `sendVerificationRequest()` 方法的源代码。请注意，我们在此方法中渲染 HTML (`html()`) 并通过网络调用 (`fetch()`) 到 Postmark 实际执行发送操作。
...
默认情况下，Auth.js 会对电子邮件地址进行规范化处理。它将地址视为不区分大小写（虽然这在技术上不符合 [RFC 2821 规范](https://datatracker.ietf.org/doc/html/rfc2821)，但在实际应用中，这样做解决的问题比带来的问题更多，例如从数据库中通过电子邮件查找用户时），同时会移除可能以逗号分隔列表形式传入的任何次要电子邮件地址。您可以通过 `Postmark` 提供者的 `normalizeIdentifier` 方法应用自定义的规范化处理。以下示例展示了默认行为：
