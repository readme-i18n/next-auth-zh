...
该登录机制首先要求用户在登录表单中提供其电子邮件地址。随后系统会向该邮箱发送一个**验证令牌**。用户需在24小时内点击邮件中的链接来"使用"该令牌并完成账户注册，否则验证令牌将失效，用户需重新申请新的验证令牌。

<Callout>
  邮件服务提供商可同时兼容[JSON Web Tokens](https://jwt.io/)和
  [数据库](/concepts/session-strategies#database-session)会话策略。无论选择哪种方式，您**都必须配置数据库**，以便Auth.js能够保存验证令牌并在用户尝试登录时进行查询。不使用数据库的情况下无法启用邮件服务提供商。
</Callout>
